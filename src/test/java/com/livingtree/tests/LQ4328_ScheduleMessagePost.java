...
public class LQ4328_ScheduleMessagePost extends ApplicationFuncs
{	

	@Test(testName="LA_12190",dataProvider="getExcelTestData",description ="Verify Overall Teacher Engagement Score")
	public void test_LQ4328ScheduleMessagePost(HashMap<String, String> rowMap) {
		try {			
			openLoginPage();
			doLogin(rowMap.get("Teacher"),rowMap.get("Password"));
			deleteAllPosts();
			String subject=getRandomString1(rowMap.get("Subject"));
			postScheduleMessage(subject ,rowMap.get("Message"),rowMap.get("GroupName") );			
			logout();
...
//			postScheduleMessageOnlyParents(subject2 ,rowMap.get("Message"),rowMap.get("GroupName") );			
//			logout();
